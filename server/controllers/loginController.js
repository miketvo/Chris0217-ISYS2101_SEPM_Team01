...
const handleLogin = async (req, res) => {
    ...
    db.query(sql1, function(err, data) {
        if(data[0].result == 1) {  // 해당 유저의 username이 있을 때 (= 데이터베이스 내 COUNT 결과 값이 1)
            const sql2 = "SELECT pw FROM users WHERE name = '" + input_name + "';";  // 비밀번호 해시 값 조회
            db.query(sql2, async function(err, rows) {
                const validPassword = await bcrypt.compare(input_pw, rows[0].pw);  // 비교문
                if(err) {
                    console.log("console1:", err);
                } else if(validPassword) {  // 비밀번호가 일치하는 경우
                    ...
                    req.session.save(function() {  // 해당 세션 저장
                        res.redirect(`/`);  // 재연결
                    });
